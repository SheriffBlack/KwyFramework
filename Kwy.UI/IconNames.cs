namespace Kwy.UI;

public class IconNames
{
    // Segoe Fluent
    public const string IconCalendar = "IconCalendar";

    public const string IconChart = "IconChart";
    public const string IconClose = "IconClose";
    public const string IconContact = "IconContact";
    public const string IconContactInfo = "IconContactInfo";
    public const string IconDelete = "IconDelete";
    public const string IconError = "IconError";
    public const string IconFlow = "IconFlow";
    public const string IconFolder = "IconFolder";
    public const string IconHome = "IconHome";
    public const string IconImport = "IconImport";
    public const string IconImportAll = "IconImportAll";
    public const string IconImportAllMirrored = "IconImportAllMirrored";
    public const string IconImportMirrored = "IconImportMirrored";
    public const string IconInfo = "IconInfo";
    public const string IconLanguage = "IconLanguage";
    public const string IconLock = "IconLock";
    public const string IconMax = "IconMax";
    public const string IconMin = "IconMin";
    public const string IconMore = "IconMore";
    public const string IconNet = "IconNet";
    public const string IconPause = "IconPause";
    public const string IconPlay = "IconPlay";
    public const string IconPowerButton = "IconPowerButton";
    public const string IconPrint = "IconPrint";
    public const string IconProcessing = "IconProcessing";
    public const string IconQuestion = "IconQuestion";
    public const string IconRestore = "IconRestore";
    public const string IconSave = "IconSave";
    public const string IconScan = "IconScan";
    public const string IconSearch = "IconSearch";
    public const string IconSendMirrored = "IconSendMirrored";
    public const string IconSet = "IconSet";
    public const string IconSuccess = "IconSuccess";
    public const string IconSwitchUser = "IconSwitchUser";
    public const string IconTouch = "IconTouch";
    public const string IconUpdateRestore = "IconUpdateRestore";
    public const string IconUserSetting = "IconUserSetting";
    public const string IconVPN = "IconVPN";

    // Geometry
    public const string IconCPU = "IconCPU";

    public const string IconAnticlockwise = "IconAnticlockwise";
    public const string IconClockise = "IconClockise";
    public const string IconDatabase = "IconDatabase";
    public const string IconDrag = "IconDrag";
    public const string IconEnglish = "IconEnglish";
    public const string IconExl = "IconExl";
    public const string IconGlasses = "IconGlasses";
    public const string IconInching = "IconInching";
    public const string IconLog = "IconLog";
    public const string IconMeasurement = "IconMeasurement";
    public const string IconRAM = "IconRAM";
    public const string IconOK = "IconOK";
    public const string IconPCI = "IconPCI";
    public const string IconPLC = "IconPLC";
    public const string IconPrintscreen = "IconPrintscreen";
    public const string IconSimplified = "IconSimplified";
    public const string IconTraditional = "IconTraditional";
    public const string IconUiModule = "IconUiModule";
    public const string IconWarning = "IconWarning";
}
