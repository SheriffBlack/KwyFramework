...
/// <summary>
/// 参数容器的基础实现。
/// 已针对 .NET 8 进行了极致性能优化，使用 <c>ReadOnlySpan&lt;char&gt;</c> 消除字符串解析时的数组内存分配。
/// <c>ParametersBase(string query)</c> 是一个高性能 query 参数解析器，用于把 <c>a=1&amp;b=2</c> 转成内部的 <c>List&lt;KeyValuePair&lt;string, object?&gt;&gt;</c> 参数列表。
/// </summary>
