    ...
    IBufferedAxisSnapshotReader
{
    ...
    public void GetMultipleAxisSnapshots(short[] axes, MotionAxisSnapshot[] destination)
    {
        ...
        int offset = 0;
        while (offset < axes.Length)
        {
            int count = 1;
            while (offset + count < axes.Length
                   && count < config.SnapshotBatchSize
                   && axes[offset + count] == axes[offset] + count)
            {
                count++;
            }

            int batchOffset = offset;
            short firstAxis = axes[offset];
            short batchCount = checked((short)count);
            Execute(() =>
            {
                SelectCard();
                ThrowIfFailed(mc.GT_GetPrfPosArray(firstAxis, batchProfilePositions, batchCount, out _), $"Read planned positions from axis {firstAxis}, count {batchCount} failed");
                ThrowIfFailed(mc.GT_GetEncPosArray(firstAxis, batchEncoderPositions, batchCount, out _), $"Read encoder positions from axis {firstAxis}, count {batchCount} failed");
                ThrowIfFailed(mc.GT_GetPrfVelArray(firstAxis, batchProfileVelocities, batchCount, out _), $"Read planned velocities from axis {firstAxis}, count {batchCount} failed");
                ThrowIfFailed(mc.GT_GetStsArray(firstAxis, batchStatuses, batchCount, out _), $"Read statuses from axis {firstAxis}, count {batchCount} failed");

                DateTimeOffset capturedAt = DateTimeOffset.Now;
                for (int index = 0; index < count; index++)
                {
                    short axis = axes[batchOffset + index];
        ...
        if (config.SnapshotSlowThresholdMilliseconds > 0
            && elapsedMilliseconds >= config.SnapshotSlowThresholdMilliseconds)
        {
            string message =
                $"Reading {axes.Length} Googol axis snapshots took {elapsedMilliseconds:F2} ms " +
                $"(threshold {config.SnapshotSlowThresholdMilliseconds:F2} ms).";
            Trace.TraceWarning(
                "[{0}/{1}] {2}",
                DeviceName,
                DeviceId,
            ...
            RaiseOperationOccurred(
                Kwy.Device.Abstractions.DeviceOperationKind.Read,
                "GetMultipleAxisSnapshots.Slow",
                true,
                message,
                properties: new Dictionary<string, string>
                {
                    ["AxisCount"] = axes.Length.ToString(System.Globalization.CultureInfo.InvariantCulture),
                    ["ElapsedMilliseconds"] = elapsedMilliseconds.ToString("F2", System.Globalization.CultureInfo.InvariantCulture),
                    ["ThresholdMilliseconds"] = config.SnapshotSlowThresholdMilliseconds.ToString("F2", System.Globalization.CultureInfo.InvariantCulture)
                });
        }
    }
    ...
    private HomeState ReadHomeStateForSnapshot(short axis)
    {
        ...
        {
            return homedAxes.Contains(axis) ? HomeState.Succeeded : HomeState.Idle;
        }

        ThrowIfFailed(mc.GT_HomeSts(axis, out ushort rawStatus), $"Get home status axis {axis} failed");
        HomeState state = rawStatus switch
        {
            0 => HomeState.Succeeded,
            1 => HomeState.Running,
            2 => HomeState.Succeeded,
            _ => HomeState.Failed
        };
