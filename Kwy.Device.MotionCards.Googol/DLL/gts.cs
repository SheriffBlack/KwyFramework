...
namespace Kwy.Device.MotionCards.Googol.DLL
{
    public class mc
    {
        ...
        public static extern short GT_GetEncPosArray(short encoder, [Out] double[] values, short count, out uint pClock);
        ...
        public static extern short GT_GetStsArray(short axis, [Out] int[] values, short count, out uint pClock);
        ...
        public static extern short GT_GetPrfPosArray(short profile, [Out] double[] values, short count, out uint pClock);

        [DllImport("gts.dll")]
        public static extern short GT_GetPrfVel(short profile, out double pValue, short count, out uint pClock);

        [DllImport("gts.dll", EntryPoint = "GT_GetPrfVel")]
        public static extern short GT_GetPrfVelArray(short profile, [Out] double[] values, short count, out uint pClock);
        ...
        // 核心函数：下发一维数组坐标进行高速比较打点
        // encoder: 编码器轴号 (通常和电机轴号一致)
        // source: 0-规划位置，1-编码器实际位置 (飞拍必须设为1)
        // outputType: 0-脉冲输出
        // startLevel: 脉冲初始电平 0-低电平，1-高电平
        // time: 触发脉冲宽度 (单位：微秒)
        // pBuf1: 一维坐标数组 (转换为脉冲数后的 int 数组)
        // count1: 数组长度
        [DllImport("gts.dll")]
        public static extern short GT_CompareData(short encoder, short source, short outputType, short startLevel, short time, ref int pBuf1, short count1, ref int pBuf2, short count2);
    }
}
